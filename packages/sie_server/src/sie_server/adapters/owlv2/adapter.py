...
Architecture: Vision Transformer encoder with contrastive text embedding
...
class Owlv2Adapter(BaseAdapter):
    ...
    def extract(
        ...
        *,
        ...
        text_queries = [f"a photo of {label.lower().strip()}" for label in labels]
    ...
    def _detect_batch(
        ...
        *,
        ...
        if pixel_values is not None and original_sizes is not None:
            ...
            # OWLv2 pads each source image to a square before resizing. Its
            # normalized boxes are therefore relative to that padded square,
            # so scaling them by the original aspect ratio would distort
            # coordinates for non-square images.
            target_sizes = torch.tensor(
                [(max(w, h), max(w, h)) for w, h in original_sizes],
                device=device,
            )
            ...
            # Match the square padding applied by Owlv2ImageProcessor.
            target_sizes = torch.tensor(
                [(max(img.width, img.height), max(img.width, img.height)) for img in images],
                device=device,
            )
    ...
    def _results_to_objects(self, result: dict[str, Any], labels: list[str]) -> list[DetectedObject]:
        ...
        label_indices_list = label_indices.detach().cpu().tolist()
        objects = []
        for i in range(n_detections):
            x1, y1, x2, y2 = boxes_list[i]
            label_idx = label_indices_list[i]
