...
change becomes a NEW versioned id. ``weights_path`` / ``package_backed`` models
are exempt (no Hub revision to drift).
"""
...
class TestValidatePinnedRevision:
    ...
    def test_package_backed_model_is_exempt(self) -> None:
