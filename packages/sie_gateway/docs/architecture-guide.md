...
Generation adds a worker-specific subject, `sie.work.{model}.{pool}.{worker_id}`, which has one additional token after the pool. That subject intentionally does not match the pool stream's `sie.work.*.{pool}` filter. The worker runtime must bind the worker-specific stream only when the local pool hosts a generation model; encode-only pools should have exactly one hot JetStream consumer, the pool consumer. This is a batching invariant, not just a resource optimization: adding an idle direct-dispatch pull loop to encode workers can perturb shared NATS intake and collapse the queue-side batch size.
