...
A pool spec may also carry `minimum_worker_count` (a per-pool warm floor, default `0`). It keeps the first request from hitting a cold VM: every 30 seconds the gateway re-emits `sie_gateway_pool_warm_floor{pool,machine_profile,bundle}` = `minimum_worker_count` for each lane the pool can name (clearing lanes that disappear), and a per-lane KEDA trigger reads it with `or vector(0)` so KEDA keeps that many machines warm. Default `0` emits nothing and leaves scale-from-zero unchanged.
